...
# NIITTECH is removed, because of IndexError: single positional indexer is out-of-bounds 'SBILIFE'
indices = ['ACC','ADANIENT','ADANIPORTS','ADANIPOWER','AMARAJABAT','AMBUJACEM']

"""
for sending_dots in range(10):
    tsend.send(".")

tsend.send("Upward Trend")
...
for x in indices:
    ...
    if ((change3 > 0) and ((day4 >100) and (day4 < 1000))):
        ...
        ultimate_1d.append(b)
        only_1d.append(x)
        if (change2 > 0):
            ultimate_2nd.append(b)
            only_2d.append(x)
            if (change1 > 0):
                ultimate_3nd.append(b)
                only_3d.append(x)
...
print("Previous 2day uptrend stocks: ",ultimate_2nd)
print("Previous 1day uptrend : ",ultimate_1d)
print(df)
...
print("final_volume = {}".format(final_volume))

########## PART 4 : SENDING MESSAGE ACCORDING TO VOLUME SORTED ie. volume 1d , 2d and 3d ###############################
def sortSecond(val): 
    return val[1]
...
for ixr in final_volume:
    ...
    if(ixr in ultimate_1d):
        final1.append[[ixr,dfxr.iloc[0][(ixr,"Volume")],"1x"]]
    
    elif (ixr in ultimate_2nd):
        final1.append([ixr,dfxr.iloc[0][(ixr,"Volume")]])

    elif (ixr in ultimate_3nd):
        final1.append([ixr,dfxr.iloc[0][(ixr,"Volume")]])

    for xvz in bx:
        dfxr = dfxr.drop(columns = [(ixr,xvz)])
    
final1.sort(key=sortSecond,reverse=True) 
    
tsend.send(".")
tsend.send(".")
tsend.send(".")
tsend.send("Stocks send according to Volume : {}".format("Day "))
tsend.send(".")
    
for sorted_value in final1:
